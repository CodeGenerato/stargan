...
def get_loader(domains,image_dir, crop_size=178, image_size=128,
    ...
    # # transform.extend([T.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.3, hue=0.5), T.RandomHorizontalFlip(), T.RandomAffine(degrees = 2)])

    if mode == 'train':
       transform.extend([T.ColorJitter(brightness=0.2, contrast=0.1, saturation=0.2, hue=0.0), T.RandomHorizontalFlip()])
